...
build:
	...
	clang $(APP_OBJC_FLAGS) -framework Cocoa $(USER_NOTIFICATIONS_FRAMEWORK) $(APP_SOURCES) -o "$(APP_DIR)/Contents/MacOS/GDriveBackupTiger"
	@set -e; ICON_WORK="$$(/usr/bin/mktemp -d "$${TMPDIR:-/tmp}/gdrive-tiger-icon.XXXXXX")"; \
		...
		install -m 644 macos/GDriveBackupTiger/AppIcon.appiconset/Contents.json \
...
test:
	...
	bash tests/nas-name-codec-integration-test.sh
	...
	@set -e; STATUS_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-status-support-test.XXXXXX")"; \
	...
	@set -e; NOTIFICATION_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-notification-support-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
			tests/notification-support-test.m macos/GDriveBackupTiger/NotificationSupport.m \
	...
	@set -e; NOTIFICATION_INTEGRATION_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-notification-integration-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Cocoa $(USER_NOTIFICATIONS_FRAMEWORK) -I macos/GDriveBackupTiger \
			...
			macos/GDriveBackupTiger/Localization.m -o "$$NOTIFICATION_INTEGRATION_TEST_BIN"; \
		"$$NOTIFICATION_INTEGRATION_TEST_BIN"; \
		./scripts/trash-path.sh "$$NOTIFICATION_INTEGRATION_TEST_BIN"
	@set -e; PROFILE_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-profile-support-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
	...
	@set -e; SETUP_HEALTH_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-setup-health-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
	...
	@set -e; RESTORE_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-restore-support-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
	...
	@set -e; CRYPT_RESTORE_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-crypt-restore-support-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
			tests/crypt-restore-support-test.m macos/GDriveBackupTiger/RestoreSupport.m \
			-o "$$CRYPT_RESTORE_SUPPORT_TEST_BIN"; \
		"$$CRYPT_RESTORE_SUPPORT_TEST_BIN"; \
		./scripts/trash-path.sh "$$CRYPT_RESTORE_SUPPORT_TEST_BIN"
	@set -e; RESTORE_UI_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-restore-ui-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Cocoa $(USER_NOTIFICATIONS_FRAMEWORK) -I macos/GDriveBackupTiger \
	...
	@set -e; DIAGNOSTICS_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-diagnostics-support-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
	...
	@set -e; UPDATE_SUPPORT_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-update-support-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
	...
	@set -e; DIAGNOSTICS_UI_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-diagnostics-ui-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Cocoa $(USER_NOTIFICATIONS_FRAMEWORK) -I macos/GDriveBackupTiger \
	...
	@set -e; CONFIG_TEST_BIN="$$(/usr/bin/mktemp "$${TMPDIR:-/tmp}/gdrive-config-test.XXXXXX")"; \
		clang $(OBJC_FLAGS) -framework Foundation -I macos/GDriveBackupTiger \
