...
- Rediscover the requested container and volume-name inventory immediately before creation and, after administrator authorization, once more inside the privileged operation immediately before `addVolume`. Recover one valid exact-name volume that appeared earlier, while duplicate requested-container records, multiple or malformed exact-name UUID records, and numeric-family names such as `GoogleDrive-Backup 1` fail closed without mutation or copy; matching names in unrelated containers do not affect the requested container.
